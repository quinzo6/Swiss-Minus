...
client.on("message", (message) => {
  ...
  let command = args.shift().toLowerCase()
    ...
    let messgaeAuthorErrorPing = message.author.id
    error
    .setTitle("Error!")
    .setAuthor(message.author.tag,message.author.avatarURL)
    .setColor('#F90B0B')
    .addField("Error:",`Hey, <@${messgaeAuthorErrorPing}> , an error occurred, that error is ${err} , please try again`)
