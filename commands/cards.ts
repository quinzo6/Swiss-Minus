...
export async function execute(client: Client, message: Message, args: string[], db: PgClient) {
    ...
    for (let count = 1; lengthe * 2 === (((await db.query("SELECT count(*) FROM information_schema.columns WHERE table_name = 'cards'")).rows[0].count - 8) as number) && lengthe >= count; count++) {
    ...
    if (aplanes.map(m => m.name).includes(carda.join('_').charAt(0).toUpperCase() + carda.join('_').slice(1))) {
        ...
        cardP
            .setColor(color)
            .setTitle(carda.join('-').charAt(0).toUpperCase() + carda.join('-').slice(1))
            .setFooter(version)
            .setTimestamp()
    ...
    if (args[0].toLowerCase() === 'upgrade') {
        ...
        let namey = aplanes.filter(plane => plane.name === (carde.join("_").charAt(0).toUpperCase() + carde.join('_').slice(1))).map(m => m.name)
        let qt = `SELECT ${namey[0]}count AS countsy FROM cards WHERE id = '${message.author.id}'`
        let lt = `SELECT ${namey[0]}levels AS levely FROM cards WHERE id = '${message.author.id}'`
        let quanity = (await db.query(qt)).rows[0].countsy
        let level = (await db.query(lt)).rows[0].levely
